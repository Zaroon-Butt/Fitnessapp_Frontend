...
const EmailOtpScreen = () => {
  ...
  const handleVerifyEmailOTP = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await AuthApi.verifyEmailOtp(email, otpString);
      
      // Store the token in onboarding context for later use in profile completion
      if (response.token) {
        updateOnboarding({ token: response.token });
      }
      ...
      showAlert({
        ...
        onClose: () => {
          // Navigate to GenderScreen to continue onboarding
  ...
  const handleResendOTP = async () => {
    ...
    try {
      ...
      await AuthApi.signUp({
        email: email,
        password: 'resend' // This should trigger OTP resend on backend
      });
      showAlert({ 
        title: 'Success', 
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.mainContent}>
        <Text style={[normal, { textAlign: 'left' }]}>EMAIL VERIFICATION</Text>

        <Text style={[regular, { marginTop: 20, textAlign: 'left' }]}>
          Check your email. We've sent you{'\n'} the verification code at {email}
        </Text>
